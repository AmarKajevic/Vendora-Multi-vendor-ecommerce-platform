...
export const userRegistration = async (
  ...
  try {
    ...
    if (existingUser) {
      return next(
        new ValidationError("User already registered with this email"),
      );
    }
    ...
    res.status(200).json({
      message:
        "OTP sent to your email. Please check your inbox for verification",
    });
  } catch (error) {
    return next(error);
  }
...
export const loginUser = async (
  ...
  try {
    ...
    //store refresh and access token in an http only cookie
...
export const resetUserPassword = async (
  ...
  try {
    ...
    //compare new paswword with existing one

    const isSamePassword = await bcrypt.compare(newPassword, user.password!);
    if (isSamePassword) {
      return next(
        new ValidationError(
          "new passoword can not be the same as the old one!",
        ),
      );
    }
...
export const updateUserPassword = async (
  ...
  try {
    ...
    if (newPassword !== confirmPassword) {
      return next(new ValidationError("new Password doesn't match"));
    }
...
export const createShop = async (
  ...
  try {
    ...
    res.status(201).json({ shop, message: "Successfully created a shop" });
  } catch (error) {
    next(error);
  }
};
...
export const LoginAdmin = async (
  ...
  try {
    ...
    //store the refresh and access tokent in an httpOnly secure cookie
    setCookie(res, "refreshToken", refreshToken);
    setCookie(res, "accessToken", accessToken);
...
// fetch layou data
export const getLayoutData = async (  req: any,
  res: Response,
  next: NextFunction,) => {
    try {
