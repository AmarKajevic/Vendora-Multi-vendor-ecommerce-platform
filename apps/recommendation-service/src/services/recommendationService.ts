...
export const recommendProducts = async (
  ...
  const processedData = preProcessData(userActions,userId, allProducts);
  ...
  const productEmbedding = tf.layers
    ...
    //dot prodcut combines user and product vectors (user-product affinity)

    const merged = tf.layers.dot({axes:1}).apply([userVector, productVector]) as tf.SymbolicTensor

    //final layer: outputs probabilty of interaction
